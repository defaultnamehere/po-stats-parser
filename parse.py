...
def pokeObjToSql(pokeObj):
	...
	query += ",".join([ "'" + str(x) + "'" for x in [pokeObj['name'],pokeObj['usage'],ab[0][0],ab[0][1],ab[1][0],ab[1][1],ab[2][0],ab[2][1],pokeObj['item'],pokeObj['nature'],mv[0],mv[1],mv[2],mv[3],st['hp'],st['atk'],st['def'],st['spa'],st['spd'],st['spe'] ]]).replace("'null'","null")
	return query
